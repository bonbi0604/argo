...
const NoticeDetail = () => {
        ...
        const fetchPostAndComments = async () => {
            try {
                const response = await api.get(
                    `https://be.argo12.duckdns.org/noticeboard/notices/${id}/`
                );
                ...
                const commentsResponse = await api.get(
                    `https://be.argo12.duckdns.org/noticeboard/notices/${id}/comments/`
                );
    ...
    const handleDelete = async () => {
        try {
            ...
            const response = await api.delete(
                `https://be.argo12.duckdns.org/noticeboard/notices/${id}/noticedelete/`
            );
    ...
    const handleDeleteComment = async (commentId) => {
        try {
            const response = await api.delete(
                `https://be.argo12.duckdns.org/noticeboard/notices/${commentId}/delete/`
            );
            if (response.status === 200 || response.status === 204) {
                setComments(
    ...
    const handleUpdateComment = async (commentId, content) => {
        try {
            const response = await api.put(
                `https://be.argo12.duckdns.org/noticeboard/notices/${commentId}/update/`,
                { content }
            );
